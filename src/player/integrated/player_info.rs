...
pub fn player_info(the_speed_of_the_account: f32, state: &PlaybackState) -> Vec<String> {
    ...
    // The row of the player is the one part of the screen that follows the
    // media that the queue changes (T-225), and the view of the chapters holds
    // the same three sentences of a name (T-227): the name of a media therefore
    // stands in `crate::logic::media_name`, and this row and that view read one
    // function. See T-224 for the key `b` of that same condition.
    vec![
        crate::logic::media_name::the_name_of_the_media(
            &state.title,

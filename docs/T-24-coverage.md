...
| **Collections** | `GET /api/libraries/:id/collections` and `GET /api/collections` give the collections. `POST /api/collections/:id/book` adds one, and `DELETE /api/collections/:id/book/:itemId` removes one | Half | The client reads, plays, adds a book (`m`), and removes a book (`X`). It cannot **make** a collection: the web page of the server does that work (T-84) |
| **Playlists** | `GET /api/libraries/:id/playlists` and `GET /api/playlists`. An entry holds `libraryItemId` and, for an episode, `episodeId`. `POST /api/playlists/:id/item` adds one, and `DELETE /api/playlists/:id/item/:itemId` removes one | Half | The same as the collections: the client adds and removes a medium (`m` and `X`), and it makes no playlist (T-84) |

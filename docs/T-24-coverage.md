...
| **Mark as finished** | `PATCH /api/me/progress/:id` with `{"isFinished":true}` | Yes | Nothing. The key `M`, and it marks a media back also. An episode of a podcast takes `PATCH /api/me/progress/:id/:episodeId` (T-219) |
| **Hide from Continue Listening** | The field `hideFromContinueListening` of `PATCH /api/me/progress/:id` | Yes | Nothing. The key `N`. A measurement on 2026-08-11 shows that the shelf of the server loses the media at once, and an episode of a podcast takes the path of the episode (T-219). **A line of a series says that a series holds no place** (T-221): the words of T-219 named a podcast for a line of a library of books |
...
| **A queue of media** | Not a function of the server. Audiobookshelf holds its queue in the web page, and it sends it to no client | Yes | Nothing. The key `n` puts the selected media at the end of the queue, and the key `q` shows the queue. `l` starts a media now, and `X` takes one out. **The queue goes on at an end, and at nothing else.** The line of the view holds the place of its own media: the percent of the user, the mark of the end, and the mark of the media that plays (T-230). `src/logic/queue.rs` |
| **The cover art** | `GET /api/items/:id/cover` gives `200` and the bytes, and `404` for an item with no cover | Yes | Nothing. T-23. The panel stands beside the description, and a series shows its books. **The store of the covers belongs to the process, and the key `R` empties it** (T-185): a request that came back with a fault kept the book with no picture for the whole life of the program, and a cover that a different client changes comes at that key |
...
| **A local copy** | `GET /api/items/:id/file/:ino/download` gives the one audio file | Yes | Nothing. `D` and `X`, for a book and for one episode (T-1, T-11). **A line that holds more than one media names the key that opens it** (T-221): a podcast of the Library view and of the view of the search, and a line of a series of the Library view, said nothing at all for the two keys |
